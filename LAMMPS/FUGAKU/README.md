...
> ## ⚠️ 本ページの内容は2024年時点の記録です（2026-08 注記）
>
> 本ページは2024年頃に実施した計算の記録であり、**それ以降の動作は確認していません。** 特に以下の点にご注意ください。
>
> - **HPCIの認証方式は、GSI認証からOAuth認証へ移行しました。** 本ページに記載している代理証明書（`myproxy-logon`）、`gsissh`、GSI-OpenSSHのDockerコンテナを用いるログイン手順は、現在は使用しません。現行の手順は[HPCIシステム利用情報・マニュアル](https://www.hpci-office.jp/for_users/hpci_info_manuals)にある『HPCIログインマニュアル OAuth対応版』および『GSI認証からOAuth認証への移行ガイド』を参照してください。
> - Spackで提供されるパッケージのバージョン（`lammps@20220623.2`、`py-pip@23.0` など）は当時のものです。`spack find -x` で現在利用可能なバージョンを確認してください。
> - ジョブスクリプトの記述やリソースグループの指定も、運用の変更を受けている可能性があります。
>
> 計算の進め方やジョブスクリプトの構成そのものは参考になると思いますが、**コマンドをそのまま実行する前に、必ず公式の最新マニュアルで確認してください。**
