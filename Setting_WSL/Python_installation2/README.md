...
> **注意**: 特定の環境の有効化を `.bashrc` に書いて常時自動化するのは避けてください。ソフトウェアのビルド（cmake等）が環境内のPythonやライブラリを意図せず拾い、「作った環境の中でしか動かないバイナリ」ができる原因になります。環境は使うときに明示的に `conda activate` で入り、ビルド作業は素のシェル（または手順が指定する環境）で行うのが原則です。
