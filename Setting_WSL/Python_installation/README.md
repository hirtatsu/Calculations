...
>
> 本リポジトリでは、Python環境を **Miniforge** に一本化しました。これから環境を構築する場合は [Pythonの環境構築（Miniforge）](../Python_installation2/README.md) を参照してください。
>
> **一本化した理由**
>
> - pyenv + venv は、Pythonのバージョン管理と仮想環境の管理を別々のツールで行う構成です。動作はしますが、科学計算で必要になるコンパイル済みライブラリ（NumPy/SciPyの数値演算バックエンドなど）の依存解決はcondaのほうが容易です。
...
>
> 本文は、すでにこの構成で環境を構築した方の参照用として残しています。記載内容は2026年1月時点のものです。
